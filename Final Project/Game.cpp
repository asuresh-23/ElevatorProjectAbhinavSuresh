...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    string load;
        while (gameFile >> load) {
            ...
            if (src != dst) {
                std::stringstream ss;
                ss << "f" << src << "t" << dst << "a" << angerDist(gen);
                Person p(ss.str());
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    int maxValue = 0;
    Floor floorNumber = building.getFloorByFloorNum(pickupFloorNum);
    int numUps = 0;
    int numDowns = 0;
    ...
    if (pickupList.length() > ELEVATOR_CAPACITY){
        ...
        for(int i = 0; i < pickupList.length(); i++){
            int counter = 0;
            char index = pickupList.at(i);
            for(int i = 0; i < pickupList.length(); i++){
                if (index == pickupList.at(i)){
                    counter++;
                }
                if (counter > 1){
                    return false;
        ...
        //checking non-neg, and setting max value
        for (int i = 0; i < pickupList.length(); i++){
            if (pickupList.at(i) - '0' < 0){
                return false;
            }
            if (pickupList.at(i) - '0' > maxValue){
                maxValue = pickupList.at(i) - '0';
            }
            //something buggy with this because it doesn't get right number of people
            if (maxValue > floorNumber.getNumPeople()){
                return false;
            }
        }
        for (int i = 0; i < pickupList.length(); i++){
            //something buggy with this because it gets people going in both directions
            Person person = floorNumber.getPersonByIndex(i);
            int targetFloor = person.getTargetFloor();
            if (targetFloor - pickupFloorNum > 0){
                numUps++;
            }
            else if (targetFloor - pickupFloorNum < 0){
                numDowns++;
            }
            else {
                return false;
            }
            if (numUps > 0 && numDowns > 0){
                return false;
            }
        }
    }
