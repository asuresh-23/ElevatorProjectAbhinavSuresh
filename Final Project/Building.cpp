...
void Building::update(Move move){
    if (!move.isPassMove()) {
        ...
        if (move.isPickupMove()){
            int peopleToBePickedUp[MAX_PEOPLE_PER_FLOOR];
            move.copyListOfPeopleToPickup(peopleToBePickedUp);

        floors[elevators[move.getElevatorId()].getCurrentFloor()].removePeople(peopleToBePickedUp, move.getNumPeopleToPickup());
            }
        }
}
